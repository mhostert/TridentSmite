...
long double flux_interpolate( std::vector<long double> &xData, std::vector<long double> &yData, long double x, long double fill_value )
{
   ...
   long double xL = xData[i], yL = yData[i], xR = xData[i+1], yR = yData[i+1];      // points on either side (unless beyond ends)

    if ( x < xL ) yR = fill_value;
    if ( x > xR ) yL = fill_value;
...
int vector3d_contains_nans(const std::vector<std::vector<std::vector<long double>>> &P){
    ...
    // std::cout<<"\n HERE\n"<< P.size() <<" "<<P[0].size()<<" "<<P[0][0].size()<< std::endl;
